"""
Hepatitis prediction service.

Follows same pattern as other disease services.
"""
...
def _resolve_model_directory(model_directory: str | Path) -> Path:
    ...
    fallback = resolve_model_directory("hepatitis_model")
...
class HepatitisService:
    def __init__(self, model_directory: str | Path) -> None:
        ...
        try:
            ...
            logger.warning("Hepatitis predictor initialization failed: %s", exc)
            self._initialized = False

    def predict(self, patient_data: dict[str, Any]) -> dict[str, Any]:
        if self.predictor is None or not self._initialized:
            # Fallback heuristic when packaged predictor is not available.
            # This ensures tests and callers receive a reasonable response
            # without requiring the binary model artifacts.
            ...
            # Simple risk heuristic: scale values to [0,1] and average
            score = 0.0
            try:
        ...
        conf_label = "Medium"
        try:
            if result.confidence >= getattr(settings, "HEPATITIS_CONFIDENCE_HIGH", 0.8):
                conf_label = "High"
            elif result.confidence >= getattr(settings, "HEPATITIS_CONFIDENCE_MEDIUM", 0.6):
                conf_label = "Medium"
            else:
                conf_label = "Low"
        except Exception:
...
_service: dict[str, HepatitisService] = {}


def get_hepatitis_service(model_directory: str | Path) -> HepatitisService:
    ...
    if getattr(app_state, "hepatitis_service", None) is not None:
        try:
            cached_dir = str(app_state.hepatitis_service.model_directory)
        except Exception:
            cached_dir = None
        if cached_dir == resolved_model_dir:
            _service[resolved_model_dir] = app_state.hepatitis_service
            return app_state.hepatitis_service

    service = HepatitisService(model_directory)
    _service[resolved_model_dir] = service

    try:
        if getattr(app_state, "hepatitis_service", None) is None:
            app_state.hepatitis_service = service
    except Exception:
        pass
