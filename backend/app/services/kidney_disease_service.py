"""
Kidney Disease prediction service.
"""
...
def _resolve_model_directory(model_directory: str | Path) -> Path:
    ...
    fallback = resolve_model_directory("kidney_disease_model")
...
class KidneyDiseaseService:
    ...
    def __init__(self, model_directory: str | Path) -> None:
        ...
        try:
            ...
            logger.warning("Kidney disease predictor initialization failed: %s", exc)
            self._initialized = False
    ...
    def predict(self, patient_data: dict[str, Any]) -> dict[str, Any]:
        ...
        conf_label = "Medium"
        try:
            if result.confidence >= settings.KIDNEY_CONFIDENCE_HIGH:
                conf_label = "High"
            elif result.confidence >= settings.KIDNEY_CONFIDENCE_MEDIUM:
                conf_label = "Medium"
            else:
                conf_label = "Low"
        except Exception:
    ...
    def health(self) -> dict[str, Any]:
        return {
            ...
            "service": "kidney_disease",
            "model_loaded": self._initialized,
            "model_directory": str(self.model_directory),
        }


_service: dict[str, KidneyDiseaseService] = {}


def get_kidney_disease_service(model_directory: str | Path) -> KidneyDiseaseService:
    ...
    if getattr(app_state, "kidney_disease_service", None) is not None:
        try:
            cached_dir = str(app_state.kidney_disease_service.model_directory)
        except Exception:
            cached_dir = None

        if cached_dir == resolved_model_dir:
            _service[resolved_model_dir] = app_state.kidney_disease_service
            return app_state.kidney_disease_service

    service = KidneyDiseaseService(model_directory)
    _service[resolved_model_dir] = service

    try:
        if getattr(app_state, "kidney_disease_service", None) is None:
            app_state.kidney_disease_service = service
    except Exception:
        pass
