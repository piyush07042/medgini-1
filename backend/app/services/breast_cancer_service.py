...
def _resolve_model_directory(model_directory: str | Path) -> Path:
    ...
    fallback = resolve_model_directory("breast_cancer_model")
...
class BreastCancerService:
    ...
    def __init__(self, model_directory: str | Path) -> None:
        ...
        try:
            ...
            logger.warning("Breast cancer predictor initialization failed: %s", exc)
            self._initialized = False
    ...
    def predict(self, patient_data: dict[str, Any]) -> dict[str, Any]:
        ...
        conf_label = "Medium"
        try:
            if result.confidence >= settings.BREAST_CANCER_CONFIDENCE_HIGH:
                conf_label = "High"
            elif result.confidence >= settings.BREAST_CANCER_CONFIDENCE_MEDIUM:
                conf_label = "Medium"
            else:
                conf_label = "Low"
        except Exception:
    ...
    def health(self) -> dict[str, Any]:
        return {
            ...
            "service": "breast_cancer",
            "model_loaded": self._initialized,
            "model_directory": str(self.model_directory),
        }


_service: dict[str, BreastCancerService] = {}


def get_breast_cancer_service(model_directory: str | Path) -> BreastCancerService:
    ...
    if getattr(app_state, "breast_cancer_service", None) is not None:
        try:
            cached_dir = str(app_state.breast_cancer_service.model_directory)
        except Exception:
            cached_dir = None

        if cached_dir == resolved_model_dir:
            _service[resolved_model_dir] = app_state.breast_cancer_service
            return app_state.breast_cancer_service

    service = BreastCancerService(model_directory)
    _service[resolved_model_dir] = service

    try:
        if getattr(app_state, "breast_cancer_service", None) is None:
            app_state.breast_cancer_service = service
    except Exception:
        pass
