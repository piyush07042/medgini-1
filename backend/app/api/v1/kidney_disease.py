...
from app.schemas.kidney_disease import KidneyDiseasePredictionRequest, KidneyDiseasePredictionResponse

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/kidney-disease", tags=["Kidney Disease"])


@router.get("/health", status_code=status.HTTP_200_OK)
async def health():
    return {"status": "healthy", "service": "kidney_disease"}


@router.post(
    "/predict",
    response_model=KidneyDiseasePredictionResponse,
    status_code=status.HTTP_200_OK,
)
async def predict(request: KidneyDiseasePredictionRequest):
    try:
        ...
        response.setdefault("disease", response.get("disease") or response.get("condition") or response.get("evaluated_condition") or "kidney_disease")
        ...
        return KidneyDiseasePredictionResponse(**response)
...
async def info():
    return {"model": "Kidney Disease", "version": "1.0.0", "status": "ready"}
