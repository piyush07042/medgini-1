...
async def health():
    return {"status": "healthy", "service": "diabetes"}
...
async def predict(request: DiabetesPredictionRequest):
    try:
        ...
        response.setdefault("disease", response.get("disease") or response.get("condition") or response.get("evaluated_condition") or "diabetes")
        ...
        return DiabetesPredictionResponse(**response)
...
async def info():
    return {"model": "Diabetes", "version": "1.0.0", "status": "ready"}
