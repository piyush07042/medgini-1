...
from app.schemas.liver_disease import LiverDiseasePredictionRequest, LiverDiseasePredictionResponse

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/liver", tags=["Liver Disease"])


@router.get("/health", status_code=status.HTTP_200_OK)
async def health():
    return {"status": "healthy", "service": "liver_disease"}


@router.post(
    "/predict",
    response_model=LiverDiseasePredictionResponse,
    status_code=status.HTTP_200_OK,
)
async def predict(request: LiverDiseasePredictionRequest):
    try:
        ...
        return LiverDiseasePredictionResponse(**response)
...
async def info():
    return {"model": "Liver Disease", "version": "1.0.0", "status": "ready"}
