...
from app.schemas.heart_failure import HeartFailurePredictionRequest, HeartFailurePredictionResponse

logger = logging.getLogger(__name__)

router = APIRouter(prefix="/heart-failure", tags=["Heart Failure"])


@router.get("/health", status_code=status.HTTP_200_OK)
async def health():
    return {"status": "healthy", "service": "heart_failure"}


@router.post(
    "/predict",
    response_model=HeartFailurePredictionResponse,
    status_code=status.HTTP_200_OK,
)
async def predict(request: HeartFailurePredictionRequest):
    try:
        supervisor = get_supervisor()
        state = AgentState()
        patient_data = request.model_dump()
        patient_data.setdefault("name", "Heart Failure Patient")
        state.patient = patient_data
        logger.info("HeartFailure API request received; patient keys=%s", list(patient_data.keys()))
        ...
        response.setdefault("disease", response.get("disease") or response.get("condition") or response.get("evaluated_condition") or "heart_failure")
        ...
        return HeartFailurePredictionResponse(**response)
...
async def info():
    return {"model": "Heart Failure", "version": "1.0.0", "status": "ready"}
